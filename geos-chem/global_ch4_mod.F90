...
!
    ...
    USE HCO_Utilities_GC_Mod, ONLY : HCO_GC_GetDiagn, HCO_GC_EvalFld
    !-zyz-
    USE ErrCode_Mod
...
!
    ...
    !+zyz+
    IF ( FIRST .and. Input_Opt%Analytical_Inv ) THEN
      CALL HCO_GC_EvalFld( Input_Opt, State_Grid, 'CH4_CLUSTERS', CLUSTERS, RC )
      IF ( RC /= GC_SUCCESS ) THEN
        ErrMsg = 'CH4_CLUSTERS not found in HEMCO data list!'
        CALL GC_Error( ErrMsg, RC, ThisLoc )
        RETURN
      ENDIF
      FIRST = .FALSE.
    ...
    ! =================================================================
      ...
      ! =================================================================
    IF ( Input_Opt%Analytical_Inv ) THEN
...
!$OMP PRIVATE( I, J)
      ...
      DO I = 1, State_Grid%NX
         !print*, i,j
         ! Only apply emission perturbation to current cluster number
         IF ( NINT(CLUSTERS(I,J)) == Input_Opt%Emis_Pert_Num ) THEN
            CH4_EMIS(I,J,1) = CH4_EMIS(I,J,1) * Input_Opt%Emis_Pert
            ! Print*, 'Analytical Inv: Scaled cluster ', &
            !          Input_Opt%Emis_Pert_Num, ' by ', &
            !          Input_Opt%Emis_Pert
         ENDIF
...
!
    ...
    !+zyz+
    ALLOCATE( CLUSTERS( State_Grid%NX, State_Grid%NY ), STAT=RC )
    CALL GC_CheckVar( 'global_ch4_mod.F90:CLUSTERS', 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
    CLUSTERS = 0e+0_fp
    !-zyz-
